...
func (g *Go) Bind(sql string, gen *tokenizer.Generated, queries []query_interpreter.Query) {
	if !slices.Contains(g.imports, "database/sql") {
		g.imports = append(g.imports, "database/sql")
	}
	if !slices.Contains(g.imports, "encoding/base64") {
		g.imports = append(g.imports, "encoding/base64")
	}
	g.sqlDump = sql
	g.generated = gen
	g.queries = queries
...
func (g *Go) Get() string {
	...
	escapedSQL := base64.StdEncoding.EncodeToString([]byte(g.sqlDump))
	sqlDump := fmt.Sprintf("func CreateDBTables(db *sql.DB) error {\n\tb, err := base64.StdEncoding.DecodeString(`%s`)\n\tif err != nil {\n\t\treturn err\n\t}\n\t_, err = db.Exec(string(b))\n\treturn err\n}\n\n", escapedSQL)

	return comments + g.headerout + imports + sqlDump + fns + "\n" + g.modelsout + g.queriesout
}
...
func (g *Go) Generate(sqlGen func(query_interpreter.Query) string) error {
	...
	if pkg == "" {
		pkg = "dist"
		color.Yellow("⚠️ GOPACKAGE not set, using \"dist\" as package name")
	}
...
func (g *Go) createModel(name string, model map[string]tokenizer.ModelConfig, keys []string) error {
	...
	for _, key := range keys {
		...
		out += fmt.Sprintf("\t%s %s%s", key, nullStr, g.getFieldType(model[realKey].Type))
